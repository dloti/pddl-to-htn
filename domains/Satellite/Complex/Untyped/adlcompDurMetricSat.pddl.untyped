...
(:requirements :durative-actions :fluents :equality :conditional-effects :negative-preconditions)
(:predicates
...
(:functions
	 (slew_time ?a ?b) (calibration_time ?a ?d) (data_capacity ?s) (data ?d ?m) (data-stored))
...
(:durative-action take_image
 ...
 :duration (= ?duration 7)
 :condition
	(and (at start (satellite ?s)) (at start (direction ?d)) (at start (instrument ?i)) (at start (mode ?m))  (over all (calibrated ?i)) (over all (on_board ?i ?s)) (over all (supports ?i ?m)) (over all (power_on ?i)) (over all (pointing ?s ?d)) (at end (power_on ?i)) (at start (>= (data_capacity ?s) (data ?d ?m))))
 :effect
	(when (at start (not (have_image ?d ?m))) (and (at end  (increase (data-stored) (data ?d ?m))) (at end  (have_image ?d ?m)) (at start  (decrease (data_capacity ?s) (data ?d ?m))))))
